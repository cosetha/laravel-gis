...
@section('content')
                <div class="container-fluid">
                        ...
                        <div class="row m-0">
                            ...
                            <form enctype="multipart/form-data" method="post" action="{{ url('') }}/dashboard/lokasi/update">
                                @csrf
                                <input type="hidden" name="id" value="{{$lokasi['id']}}"> <br/>
                                <div class="form-row">  
                                    <div class="form-group col-md-6">
                                        <label for="namaTempat">Nama Tempat:</label>
                                        <input type="text" class="form-control" id="namaTempat" placeholder="Nama Tempat" name="nama" required value = "{{$lokasi['nama']}}">
                                    </div> 
                                </div>
                                <div class="form-group ">
                                        <label for="lokasiTempat">Lokasi Tempat:</label>
                                        <input type="text" class="form-control" id="lokasiTempat" placeholder="Lokasi Tempat" name="lokasi" required value = "{{$lokasi['lokasi']}}">
                                    </div>
                                
                                <div class="form-row">
                                    <div class="form-group col-md-5">
                                        <label for="long">Longitude</label>
                                        <input type="text" class="form-control" id="long" name="long"  required readonly value = "{{$lokasi['long']}}"> 
                                    </div>
                                    <div class="form-group col-md-5">
                                        <label for="lat">Latitude</label>
                                        <input type="text" id="lat" class="form-control" name="lat"  required readonly value = "{{$lokasi['lat']}}">                                    
                                    </div>
                                </div>
                                <div class="form-group">
